...
app.get('/', (req, res) => {
  res.json({
    status: 'ok',
    message: 'FittingRoom backend is running.',
    garmentPipelineVersion: GARMENT_PIPELINE_VERSION,
  });
});
...
function normalizeGarmentType(garmentType) {
  ...
  const canonical = GARMENT_TYPE_ALIASES[normalized] || normalized;
  return Object.prototype.hasOwnProperty.call(GARMENT_LABELS, canonical) ? canonical : 'shirt';
}

function inferGarmentTypeFromText(text = '') {
  const source = String(text || '').toLowerCase();
  if (!source) return null;

  const aliasMatched = Object.entries(GARMENT_TYPE_ALIASES).find(([alias]) => {
    const token = alias.toLowerCase();
    return source.includes(token);
  });
  if (aliasMatched) {
    return normalizeGarmentType(aliasMatched[1]);
  }

  for (const [type, labels] of Object.entries(GARMENT_LABELS)) {
    if (!Array.isArray(labels)) continue;
    for (const label of labels) {
      const token = String(label || '').toLowerCase().replace(/[-_]+/g, ' ').trim();
      if (!token) continue;
      if (source.includes(token) || source.includes(token.replace(/\s+/g, ''))) {
        return normalizeGarmentType(type);
      }
    }
  }
...
function resolveRequestedGarmentType(rawGarmentType, fileName = '') {
  const normalizedRequested = normalizeGarmentType(rawGarmentType);
  if (normalizedRequested !== 'shirt') {
    return normalizedRequested;
  }

  // If UI/default falls back to shirt, recover from filename hints (e.g. "Jeans.png").
  const inferredFromName = inferGarmentTypeFromText(fileName);
  if (inferredFromName && inferredFromName !== 'shirt') {
...
async function isLikelyUniformBackground(buffer) {
  ...
  // Use corner regions only so garments that touch the bottom/side edges
  // don't contaminate the background colour sample and cause a false negative.
  const cornerBand = Math.max(2, Math.round(Math.min(width, height) * 0.06));
  let samples = 0;
  let sumR = 0;
  let sumG = 0;
  let sumB = 0;
  ...
  const quantize = (value) => Math.max(0, Math.min(15, Math.floor(value / 16)));
  ...
  const varR = Math.max(0, (sumSqR / samples) - (meanR * meanR));
  const varG = Math.max(0, (sumSqG / samples) - (meanG * meanG));
  const varB = Math.max(0, (sumSqB / samples) - (meanB * meanB));
  const stdAvg = (Math.sqrt(varR) + Math.sqrt(varG) + Math.sqrt(varB)) / 3;

  let dominantBucket = 0;
  for (const count of bucketCounts.values()) {
    if (count > dominantBucket) dominantBucket = count;
  }
  const dominantRatio = dominantBucket / samples;

  // Uniform-border trigger for any color background, not just white.
  return dominantRatio >= 0.5 && stdAvg <= 22;
}
...
async function createGarmentCutout(buffer, garmentType = 'shirt', mimeType = '') {
  ...
  try {
    const uniformBackground = await isLikelyUniformBackground(orientedBuffer);
    if (uniformBackground) {
      const sourceMeta = await sharp(orientedBuffer).metadata();
      const sourceArea = Math.max(1, (sourceMeta.width || 1) * (sourceMeta.height || 1));
      const colorKeyResult = await createColorKeyCutout(orientedBuffer);
      const cutoutArea = Math.max(0, (colorKeyResult.cutout?.width || 0) * (colorKeyResult.cutout?.height || 0));
      const ratio = cutoutArea / sourceArea;

      if (ratio >= 0.03 && ratio <= 0.95) {
        return colorKeyResult;
      }
    }
  } catch (uniformPathError) {
    console.warn('Uniform-background cutout path failed, falling back to model cutout:', uniformPathError.message);
  }
...
async function refineGarmentCutoutWithSharp(cutoutBuffer, garmentType = 'shirt') {
  ...
  // Opening pass removed — it was severing thin connections in dresses/skirts with narrow waists.
  ...
  // Remove detached micro-islands and fill tiny enclosed pinholes so the final
  // silhouette is continuous without white speck artifacts.
  const cleanedMask = new Uint8Array(trimmedMask);
  const componentVisited = new Uint8Array(width * height);
  const componentQueue = [];
  let componentQueueHead = 0;
  let largestComponentArea = 0;
  const componentsToEvaluate = [];

  for (let start = 0; start < cleanedMask.length; start += 1) {
    if (!cleanedMask[start] || componentVisited[start]) continue;
    ...
    while (componentQueueHead < componentQueue.length) {
      ...
      if (x > 0) {
        ...
        if (cleanedMask[left] && !componentVisited[left]) {
          componentVisited[left] = 1;
          componentQueue.push(left);
        }
      }
      if (x < width - 1) {
        const right = current + 1;
        if (cleanedMask[right] && !componentVisited[right]) {
          componentVisited[right] = 1;
          componentQueue.push(right);
        }
      }
      if (y > 0) {
        const up = current - width;
        if (cleanedMask[up] && !componentVisited[up]) {
          componentVisited[up] = 1;
          componentQueue.push(up);
        }
      }
      if (y < height - 1) {
        const down = current + width;
        if (cleanedMask[down] && !componentVisited[down]) {
          componentVisited[down] = 1;
          componentQueue.push(down);
        }
      }
    }

    largestComponentArea = Math.max(largestComponentArea, area);
    componentsToEvaluate.push({ pixels, area });
  }

  const minIslandArea = Math.max(18, Math.round(largestComponentArea * 0.0025));
  for (const component of componentsToEvaluate) {
    if (component.area >= minIslandArea) continue;
  ...
  const holeQueue = [];
  let holeQueueHead = 0;
  const maxHoleArea = Math.max(24, Math.round(Math.max(1, largestComponentArea) * 0.003));

  for (let start = 0; start < cleanedMask.length; start += 1) {
    if (cleanedMask[start] || holeVisited[start]) continue;

    holeQueue.length = 0;
    holeQueueHead = 0;
    holeQueue.push(start);
    holeVisited[start] = 1;

    const pixels = [];
    let touchesBorder = false;

    while (holeQueueHead < holeQueue.length) {
      const current = holeQueue[holeQueueHead++];
      const x = current % width;
      const y = Math.floor(current / width);
      pixels.push(current);
      ...
      if (x > 0) {
        const left = current - 1;
        if (!cleanedMask[left] && !holeVisited[left]) {
          holeVisited[left] = 1;
          holeQueue.push(left);
        }
      }
      if (x < width - 1) {
        const right = current + 1;
        if (!cleanedMask[right] && !holeVisited[right]) {
          holeVisited[right] = 1;
          holeQueue.push(right);
        }
      }
      if (y > 0) {
        const up = current - width;
        if (!cleanedMask[up] && !holeVisited[up]) {
          holeVisited[up] = 1;
          holeQueue.push(up);
        }
      }
      if (y < height - 1) {
        const down = current + width;
        if (!cleanedMask[down] && !holeVisited[down]) {
          holeVisited[down] = 1;
          holeQueue.push(down);
        }
      }
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      ...
      if (!cleanedMask[index]) {
        outputRaw[offset] = 0;
        outputRaw[offset + 1] = 0;
        outputRaw[offset + 2] = 0;
...
function buildRowSegments(mask, width, height) {
...
function buildStitchableBoundaryMask(mask, width, height, garmentType = 'shirt') {
  const stitchable = new Uint8Array(mask.length);
  ...
  const normalizedType = normalizeGarmentType(garmentType);
  ...
  const isBoundaryPixel = (x, y) => {
    ...
    const leftOpen = x === 0 || !mask[index - 1];
    const rightOpen = x === width - 1 || !mask[index + 1];
    const topOpen = y === 0 || !mask[index - width];
    const bottomOpen = y === height - 1 || !mask[index + width];
    return leftOpen || rightOpen || topOpen || bottomOpen;
  };

  // Keep only boundary pixels adjacent to exterior background, then stitch all of them.
  const exteriorBg = new Uint8Array(mask.length);
  const queue = new Int32Array(mask.length);
  let qStart = 0;
  let qEnd = 0;

  const seedExterior = (x, y) => {
    if (x < 0 || x >= width || y < 0 || y >= height) return;
    const index = y * width + x;
    if (mask[index] || exteriorBg[index]) return;
    exteriorBg[index] = 1;
    queue[qEnd++] = index;
  };
  ...
  for (let x = 0; x < width; x += 1) {
    seedExterior(x, 0);
    seedExterior(x, height - 1);
  }
  for (let y = 0; y < height; y += 1) {
    seedExterior(0, y);
    seedExterior(width - 1, y);
  }
  ...
  while (qStart < qEnd) {
    ...
    seedExterior(cx - 1, cy);
    seedExterior(cx + 1, cy);
    seedExterior(cx, cy - 1);
    seedExterior(cx, cy + 1);
  }
  ...
  const touchesExterior = (x, y) => {
    ...
    for (const [nx, ny] of neighbors) {
      ...
      const nIndex = ny * width + nx;
      if (!mask[nIndex] && exteriorBg[nIndex]) return true;
    }
    return false;
  };
  ...
  for (let y = minY; y <= maxY; y += 1) {
    for (let x = minX; x <= maxX; x += 1) {
      if (isBoundaryPixel(x, y) && touchesExterior(x, y)) {
        stitchable[y * width + x] = 1;
      }
    }
  }

  const clearByNormalizedRegion = (predicate) => {
    for (let y = minY; y <= maxY; y += 1) {
      const v = (y - minY) / spanY;
      for (let x = minX; x <= maxX; x += 1) {
        const index = y * width + x;
        if (!stitchable[index]) continue;
  ...
  const clearNeck = () => {
    // Must be slightly LARGER than carveNeckOpening (0.18 deep, 0.16 halfWidth)
    // so the rim pixels left by the carve are fully inside the exclusion zone.
    clearByNormalizedRegion((u, v) => {
      if (v > 0.21) return false;
      const t = v / 0.21;
      const halfWidth = 0.20 * Math.sqrt(1 - t * t);
      return Math.abs(u - 0.5) <= halfWidth;
    });
  };

  const clearArms = () => {
    // Only cut armhole threads in the lateral-underarm zone so sleeve-cap seams stay stitched.
    clearByNormalizedRegion((u, v) => v >= 0.2 && v <= 0.56 && (u <= 0.2 || u >= 0.8));
  };

  const clearWaist = () => {
    clearByNormalizedRegion((u, v) => v <= 0.09);
  };

  const clearBottomHem = () => {
    clearByNormalizedRegion((u, v) => v >= 0.94);
  };

  const clearLegOpenings = () => {
    // Clear the full bottom of both leg tubes: outer edges, inseam edges, and bottom hems.
    // Using v >= 0.87 ensures both the outer and inner (inseam-side) openings of each leg
    // are left unstitched, preventing the leg bottoms from being sealed shut.
    clearByNormalizedRegion((u, v) => v >= 0.87);
  };

  // Clear just the crotch-inseam gap so the two leg tubes don't get stitched across to each
  // other at the top of the inseam.  Only the narrow central pinch (u 0.38–0.62, v 0.48–0.64)
  // is cleared; the rest of the inseam side-seams remain stitchable so each tube closes correctly.
  const clearCrotchGap = () => {
    clearByNormalizedRegion((u, v) => v >= 0.48 && v <= 0.64 && u >= 0.38 && u <= 0.62);
  };

  const topTypes = new Set(['shirt', 'tshirt', 'blouse', 'jacket', 'hoodie', 'sweater', 'suit', 'coat']);
  if (topTypes.has(normalizedType)) {
    clearNeck();
    clearArms();
    clearBottomHem();
    return stitchable;
  ...
  if (normalizedType === 'skirt') {
    clearWaist();
    clearBottomHem();
    return stitchable;
  }

  if (normalizedType === 'pants' || normalizedType === 'shorts') {
    clearWaist();
    clearLegOpenings();
    clearCrotchGap();
    return stitchable;
  ...
  if (normalizedType === 'romper' || normalizedType === 'jumpsuit') {
    clearNeck();
    clearArms();
    clearLegOpenings();
    return stitchable;
...
function resolveTopGarmentBounds(mask, width, height) {
  let minX = width;
  let minY = height;
  let maxX = -1;
  let maxY = -1;
  ...
  const spanX = Math.max(1, maxX - minX);
  const spanY = Math.max(1, maxY - minY);
  const sampleEndY = Math.min(maxY, minY + Math.max(3, Math.round(spanY * 0.2)));
  const rowCenters = [];
  let topNarrowestWidth = Number.POSITIVE_INFINITY;

  for (let y = minY; y <= sampleEndY; y += 1) {
    let rowMinX = width;
    let rowMaxX = -1;
    for (let x = 0; x < width; x += 1) {
      const index = y * width + x;
    ...
    if (rowMaxX >= rowMinX) {
      ...
      topNarrowestWidth = Math.min(topNarrowestWidth, rowWidth);
      rowCenters.push((rowMinX + rowMaxX) * 0.5);
    }
  }

  const centerX = rowCenters.length > 0
    ? rowCenters.reduce((sum, value) => sum + value, 0) / rowCenters.length
    : (minX + maxX) * 0.5;
  const safeTopNarrowestWidth = Number.isFinite(topNarrowestWidth)
    ? topNarrowestWidth
    : Math.max(1, Math.round(spanX * 0.6));
  ...
  return {
    ...
    spanX,
    spanY,
    topNarrowestWidth: safeTopNarrowestWidth,
    centerX,
  };
}

function buildNeckExclusionMask(mask, width, height, garmentType = 'shirt') {
  const exclusion = new Uint8Array(mask.length);
  const normalizedType = normalizeGarmentType(garmentType);
  const topHoleTypes = new Set(['shirt', 'tshirt', 'blouse', 'jacket', 'hoodie', 'sweater', 'suit', 'coat', 'dress', 'romper', 'jumpsuit']);
  if (!topHoleTypes.has(normalizedType)) {
    return exclusion;
  }

  const bounds = resolveTopGarmentBounds(mask, width, height);
  if (!bounds) {
    return exclusion;
  }

  const neckDepth = Math.min(8, Math.max(3, Math.round(bounds.spanY * 0.12)));
  const neckEndY = Math.min(bounds.maxY, bounds.minY + neckDepth);
  const baseHalfWidth = Math.max(
    3,
    Math.round(Math.min(bounds.spanX * 0.08, bounds.topNarrowestWidth * 0.12, 7))
  );

  for (let y = bounds.minY; y <= neckEndY; y += 1) {
    const t = (y - bounds.minY) / Math.max(1, neckEndY - bounds.minY);
    const halfWidth = Math.max(2, Math.round(baseHalfWidth * (1 - t * 0.25)));
    const startX = Math.max(bounds.minX, Math.floor(bounds.centerX - halfWidth));
    const endX = Math.min(bounds.maxX, Math.ceil(bounds.centerX + halfWidth));
...
function carveNeckOpening(mask, width, height, garmentType = 'shirt') {
  const normalizedType = normalizeGarmentType(garmentType);
  const topHoleTypes = new Set(['shirt', 'tshirt', 'blouse', 'jacket', 'hoodie', 'sweater', 'suit', 'coat', 'dress', 'romper', 'jumpsuit']);
  if (!topHoleTypes.has(normalizedType)) {
    return;
  ...
  // Neck/head hole: ~30% of body width, ~18% of body height, oval-tapered.
  const neckDepth = Math.round(bounds.spanY * 0.18);
  const neckEndY = Math.min(bounds.maxY, bounds.minY + neckDepth);
  const neckHalfWidthMax = Math.round(bounds.spanX * 0.16); // 32% total width at widest

  for (let y = bounds.minY; y <= neckEndY; y += 1) {
    // Elliptical taper: wide at top, narrows toward neckEndY
    const t = (y - bounds.minY) / Math.max(1, neckEndY - bounds.minY);
    const rowHalfWidth = Math.max(2, Math.round(neckHalfWidthMax * Math.sqrt(1 - t * t)));
    const startX = Math.max(bounds.minX, Math.floor(bounds.centerX - rowHalfWidth));
    const endX = Math.min(bounds.maxX, Math.ceil(bounds.centerX + rowHalfWidth));
...
async function createApproxGarment3DModelFromCutout(buffer, garmentType = 'shirt', mimeType = '', existingCutoutResult = null) {
  ...
  // ---- Solid interior fill (hole-fill by exterior flood) ----
  // Run BEFORE component selection so each leg/panel is already solid when
  // we score components by area. Interior holes caused by semi-transparent
  // fabric pixels (dark denim etc.) are enclosed by the outer contour and
  // cannot be reached from the image border — fill them now.
  {
    const exteriorReached = new Uint8Array(pixelCount);
    const holeQ = new Int32Array(pixelCount);
    let hqStart = 0;
    let hqEnd = 0;

    const seedBorder = (idx) => {
      if (!cleanedMask[idx] && !exteriorReached[idx]) {
        exteriorReached[idx] = 1;
        holeQ[hqEnd++] = idx;
      }
    };
    for (let x = 0; x < width; x += 1) {
    ...
    while (hqStart < hqEnd) {
      const cur = holeQ[hqStart++];
      const cx = cur % width;
      const cy = Math.floor(cur / width);
      if (cx > 0) { const nb = cur - 1; if (!cleanedMask[nb] && !exteriorReached[nb]) { exteriorReached[nb] = 1; holeQ[hqEnd++] = nb; } }
      if (cx < width - 1) { const nb = cur + 1; if (!cleanedMask[nb] && !exteriorReached[nb]) { exteriorReached[nb] = 1; holeQ[hqEnd++] = nb; } }
      if (cy > 0) { const nb = cur - width; if (!cleanedMask[nb] && !exteriorReached[nb]) { exteriorReached[nb] = 1; holeQ[hqEnd++] = nb; } }
      if (cy < height - 1) { const nb = cur + width; if (!cleanedMask[nb] && !exteriorReached[nb]) { exteriorReached[nb] = 1; holeQ[hqEnd++] = nb; } }
    }
  ...
  // Keep primary connected silhouette(s). For pants/shorts, preserve up to two components
  // (left leg and right leg) when they are disconnected; for other garments keep only the
  // single best-scoring component to remove detached background fragments.
  // Keep the largest connected silhouette(s) by raw area.
  // No center-distance bias — legs and off-center garment parts must not be penalised.
  {
    const visited = new Uint8Array(pixelCount);
    const queueLocal = new Int32Array(pixelCount);
    const centerX = width / 2;
    const allComponents = [];
    ...
    for (let start = 0; start < pixelCount; start += 1) {
      ...
      while (qStart < qEnd) {
        const current = queueLocal[qStart++];
        const x = current % width;
        const y = Math.floor(current / width);
        area += 1;
        sumX += x;
        ...
        if (x > 0) {
          const left = current - 1;
          if (cleanedMask[left] && !visited[left]) {
            visited[left] = 1;
            queueLocal[qEnd++] = left;
          }
        }
        if (x < width - 1) {
          const right = current + 1;
          if (cleanedMask[right] && !visited[right]) {
            visited[right] = 1;
            queueLocal[qEnd++] = right;
          }
        }
        if (y > 0) {
          const up = current - width;
          if (cleanedMask[up] && !visited[up]) {
            visited[up] = 1;
            queueLocal[qEnd++] = up;
          }
        }
        if (y < height - 1) {
          const down = current + width;
          if (cleanedMask[down] && !visited[down]) {
            visited[down] = 1;
            queueLocal[qEnd++] = down;
          }
        }
      }

      const centroidX = sumX / Math.max(1, area);
      allComponents.push({ start, area, centroidX });
    }

    // Sort by raw area descending — largest blob wins regardless of position.
    allComponents.sort((compA, compB) => compB.area - compA.area);
    const keepStarts = [];
    if (allComponents.length > 0) {
      keepStarts.push(allComponents[0].start);

      // For pants/shorts keep a second blob if it looks like the other leg:
      // area >= 20 % of the best AND it sits on the opposite horizontal half.
      const normalizedGarmentTypeForComponents = normalizeGarmentType(garmentType);
      const isPairedLegGarment = normalizedGarmentTypeForComponents === 'pants'
        || normalizedGarmentTypeForComponents === 'shorts';

      if (isPairedLegGarment && allComponents.length >= 2) {
        const best = allComponents[0];
        const second = allComponents[1];
        const horizontallyOpposite = (best.centroidX < centerX) !== (second.centroidX < centerX);
        const sizeRatio = second.area / Math.max(1, best.area);
        if (sizeRatio >= 0.20 && (horizontallyOpposite || sizeRatio >= 0.45)) {
          keepStarts.push(second.start);
        }
      }
    }

    if (keepStarts.length > 0) {
      const keep = new Uint8Array(pixelCount);

      for (const kStart of keepStarts) {
        let qStart = 0;
        let qEnd = 0;
        queueLocal[qEnd++] = kStart;
        keep[kStart] = 1;
        ...
        while (qStart < qEnd) {
          ...
          if (x < width - 1) {
            const right = current + 1;
            if (cleanedMask[right] && !keep[right]) {
              keep[right] = 1;
              queueLocal[qEnd++] = right;
            }
          }
  ...
  // Smooth tiny contour spikes and pinholes before seam extraction.
  // Skip erosion for lower-body garments: the narrow crotch V-shape has too few
  // neighbours and the passes would split the connected pants into two separate legs.
  const normalizedTypeForSmoothing = normalizeGarmentType(garmentType);
  const skipErosion = normalizedTypeForSmoothing === 'pants'
    || normalizedTypeForSmoothing === 'shorts'
    || normalizedTypeForSmoothing === 'skirt';
  for (let pass = 0; pass < (skipErosion ? 0 : 2); pass += 1) {
    const nextMask = new Uint8Array(cleanedMask);
    for (let y = 1; y < height - 1; y += 1) {
      for (let x = 1; x < width - 1; x += 1) {
        const index = y * width + x;
        let neighbors = 0;
        for (let oy = -1; oy <= 1; oy += 1) {
          for (let ox = -1; ox <= 1; ox += 1) {
            if (ox === 0 && oy === 0) continue;
            if (cleanedMask[(y + oy) * width + (x + ox)]) {
              neighbors += 1;
            }
          }
        }

        if (cleanedMask[index] && neighbors <= 2) {
          nextMask[index] = 0;
        } else if (!cleanedMask[index] && neighbors >= 7) {
          nextMask[index] = 1;
        }
      }
    }
    cleanedMask.set(nextMask);
  }

  // Lower-body garments are sensitive to 1-2 pixel cracks from cutout noise.
  // Repair tiny gaps so the generated panel surfaces are continuous instead of fragmented islands.
  if (normalizedTypeForSmoothing === 'pants' || normalizedTypeForSmoothing === 'shorts') {
    const repairedMask = new Uint8Array(cleanedMask);

    // Fill pinholes and micro-cracks with a conservative bridge rule.
    for (let y = 1; y < height - 1; y += 1) {
      for (let x = 1; x < width - 1; x += 1) {
        const index = y * width + x;
        ...
        if (orth >= 3 || (orth >= 2 && diag >= 2) || horizontalBridge || verticalBridge) {
          repairedMask[index] = 1;
        }
      }
    }

    // Keep only primary repaired component(s) to avoid isolated specks.
    const keep = new Uint8Array(pixelCount);
    const visited = new Uint8Array(pixelCount);
    const queueRepair = new Int32Array(pixelCount);
    const components = [];

    for (let start = 0; start < pixelCount; start += 1) {
      if (!repairedMask[start] || visited[start]) continue;

      let qStart = 0;
      let qEnd = 0;
      queueRepair[qEnd++] = start;
      visited[start] = 1;
      let area = 0;

      while (qStart < qEnd) {
        const current = queueRepair[qStart++];
        area += 1;
        const cx = current % width;
        const cy = Math.floor(current / width);
        ...
        if (cx > 0) {
          const left = current - 1;
          if (repairedMask[left] && !visited[left]) {
            visited[left] = 1;
            queueRepair[qEnd++] = left;
          }
        }
        if (cx < width - 1) {
          const right = current + 1;
          if (repairedMask[right] && !visited[right]) {
            visited[right] = 1;
            queueRepair[qEnd++] = right;
          }
        }
        if (cy > 0) {
          const up = current - width;
          if (repairedMask[up] && !visited[up]) {
            visited[up] = 1;
            queueRepair[qEnd++] = up;
          }
        }
        if (cy < height - 1) {
          const down = current + width;
          if (repairedMask[down] && !visited[down]) {
            visited[down] = 1;
            queueRepair[qEnd++] = down;
          }
        }
      }

      components.push({ start, area });
    }

    components.sort((first, second) => second.area - first.area);
    const keepCount = Math.min(2, components.length);
    for (let comp = 0; comp < keepCount; comp += 1) {
      const start = components[comp].start;
      let qStart = 0;
      let qEnd = 0;
      queueRepair[qEnd++] = start;
      keep[start] = 1;
      ...
      while (qStart < qEnd) {
        ...
        if (cx > 0) {
          const left = current - 1;
          if (repairedMask[left] && !keep[left]) {
            keep[left] = 1;
            queueRepair[qEnd++] = left;
          }
        }
        if (cx < width - 1) {
          const right = current + 1;
          if (repairedMask[right] && !keep[right]) {
            keep[right] = 1;
            queueRepair[qEnd++] = right;
          }
        }
        ...
        if (cy < height - 1) {
          const down = current + width;
          if (repairedMask[down] && !keep[down]) {
            keep[down] = 1;
            queueRepair[qEnd++] = down;
          }
        }
    ...
    for (let index = 0; index < pixelCount; index += 1) {
      cleanedMask[index] = keep[index] ? 1 : 0;
    }
  }
  ...
  const stitchableBoundaryMask = buildStitchableBoundaryMask(cleanedMask, width, height, garmentType);
  const neckExclusionMask = buildNeckExclusionMask(cleanedMask, width, height, garmentType);
  const stitchedBoundaryMask = new Uint8Array(pixelCount);
  let boundaryPixelCount = 0;
  let stitchableBoundaryPixelCount = 0;
  let excludedOpeningPixelCount = 0;
  const positions = [];
  const uvs = [];
  const indices = [];
  const seamWallIndices = [];
  const panelIds = [];
  const surfaceSides = [];
  ...
  let garmentColorSumB = 0;

  const aspect = width / Math.max(1, height);
  const scaleY = 1.35;
  const scaleX = scaleY * aspect;

  const seamDepth = 0.0022;

  // Pants/shorts need a wider pinned waistband so the garment hangs stably from the hips.
  const normalizedGarmentTypePinning = normalizeGarmentType(garmentType);
  const isLowerBodyGarmentPinning = normalizedGarmentTypePinning === 'pants'
    || normalizedGarmentTypePinning === 'shorts';
  const pinSolidThreshold = isLowerBodyGarmentPinning
    ? Math.round(height * 0.10)
    : Math.max(2, Math.round(height * 0.06));
  const pinBoundaryThreshold = isLowerBodyGarmentPinning
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      ...
      if (isBoundary) {
        ...
        if (stitchableBoundaryMask[index] && !neckExclusionMask[index]) {
          stitchableBoundaryPixelCount += 1;
        } else {
          excludedOpeningPixelCount += 1;
        }
      }
    }
  }
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      ...
      const green = data[offset + 1];
      const blue = data[offset + 2];
      garmentColorSampleCount += 1;
      garmentColorSumR += red;
      garmentColorSumG += green;
      garmentColorSumB += blue;
      const luminance = (0.2126 * red + 0.7152 * green + 0.0722 * blue) / 255;

      const distanceNorm = Math.min(1, (distance[index] || 0) / Math.max(1, maxDistance));
      const wrinkle = (0.5 - luminance) * 0.02;

      // Panel effect: gentle depth response only; panel IDs are explicit front/back.
      const panelU = x / Math.max(1, width - 1);
      const seamRidge = Math.sin(panelU * Math.PI) * seamDepth;

      // Slight drape sag towards the lower body to mimic cloth hanging.
      const v = y / Math.max(1, height - 1);
      const drapeSag = Math.pow(v, 1.8) * 0.018;

      const frontZ = 0.006 + distanceNorm * 0.02 + wrinkle * 0.35 + seamRidge - drapeSag * 0.35;
      const backZ = -0.006 - distanceNorm * 0.016 + wrinkle * 0.18 + seamRidge * 0.35 - drapeSag * 0.28;
      ...
      panelIds.push(0);
      surfaceSides.push(0);
      pinnedVertices.push(y <= pinSolidThreshold || (boundaryMask[index] && y <= pinBoundaryThreshold));
      ...
      const isStitchedBoundary = boundaryMask[index] && stitchableBoundaryMask[index] && !neckExclusionMask[index];
  ...
  for (let y = 0; y < height - 1; y += 1) {
    for (let x = 0; x < width - 1; x += 1) {
      ...
      if (cleanedMask[i00] && cleanedMask[i10] && stitchedBoundaryMask[i00] && stitchedBoundaryMask[i10]) {
        ...
        if (f0 >= 0 && f1 >= 0 && b0 >= 0 && b1 >= 0) {
          seamWallIndices.push(f0, f1, b1);
          seamWallIndices.push(f0, b1, b0);
        }
      }

      if (cleanedMask[i00] && cleanedMask[i01] && stitchedBoundaryMask[i00] && stitchedBoundaryMask[i01]) {
        const f0 = frontVertexMap[i00];
        const f1 = frontVertexMap[i01];
        const b0 = backVertexMap[i00];
        const b1 = backVertexMap[i01];
        if (f0 >= 0 && f1 >= 0 && b0 >= 0 && b1 >= 0) {
          seamWallIndices.push(f0, b1, f1);
          seamWallIndices.push(f0, b0, b1);
        }
      }
    }
  }
  ...
  if (seamWallIndices.length > 0) {
    indices.push(...seamWallIndices);
  }

  const garmentColor = garmentColorSampleCount > 0
    ? {
        r: Math.max(0, Math.min(255, Math.round(garmentColorSumR / garmentColorSampleCount))),
        g: Math.max(0, Math.min(255, Math.round(garmentColorSumG / garmentColorSampleCount))),
        b: Math.max(0, Math.min(255, Math.round(garmentColorSumB / garmentColorSampleCount))),
      }
    : null;
  ...
  const garmentModel = {
    ...
    seamDiagnostics: {
      boundaryPixelCount,
      stitchableBoundaryPixelCount,
      excludedOpeningPixelCount,
      seamWallTriangleCount: Math.floor(seamWallIndices.length / 3),
    },
    geometryGroups: {
      mainIndexCount,
      seamWallIndexCount: seamWallIndices.length,
    },
    garmentColor,
  };
...
async function createGarment3DModel(buffer, garmentType = 'shirt', mimeType = '') {
  ...
  // Use only the local panel-based cloth generator so seam behavior is deterministic.
  return createApproxGarment3DModelFromCutout(buffer, garmentType, mimeType, cutoutResult);
}
...
async function createColorKeyCutout(buffer) {
  ...
  if (width < 2 || height < 2 || channels < 4) {
    ...
    return {
      ...
      cutout: { width, height, offsetX: 0, offsetY: 0 },
    };
  }

  const outputRaw = Buffer.from(data);

  // Sample background color from image corners (average of 4 corner regions).
  const cornerBand = Math.max(3, Math.round(Math.min(width, height) * 0.04));
  let sumR = 0; let sumG = 0; let sumB = 0; let samples = 0;
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      const isCorner =
        (x < cornerBand || x >= width - cornerBand) &&
  ...
  const bgR = samples > 0 ? sumR / samples : 255;
  const bgG = samples > 0 ? sumG / samples : 255;
  const bgB = samples > 0 ? sumB / samples : 255;
  // Allow slightly more tolerance for backgrounds that have subtle gradients or shadows.
  const bgTolerance = 55;
  const bgToleranceSq = bgTolerance * bgTolerance;

  // Exterior flood-fill: BFS from all border pixels through "background-similar" pixels.
  // Any pixel not reached is interior (garment).
  const isBackgroundColor = (r, g, b) => {
    const dr = r - bgR; const dg = g - bgG; const db = b - bgB;
    return (dr * dr + dg * dg + db * db) <= bgToleranceSq;
  };

  const exteriorReached = new Uint8Array(width * height);
  const bfsQueue = new Int32Array(width * height);
  let bfsStart = 0; let bfsEnd = 0;

  const seedBorder = (idx) => {
    const off = idx * channels;
    if (!exteriorReached[idx] && isBackgroundColor(data[off], data[off + 1], data[off + 2])) {
      exteriorReached[idx] = 1;
      bfsQueue[bfsEnd++] = idx;
    }
  };
  for (let x = 0; x < width; x += 1) { seedBorder(x); seedBorder((height - 1) * width + x); }
  for (let y = 0; y < height; y += 1) { seedBorder(y * width); seedBorder(y * width + (width - 1)); }

  while (bfsStart < bfsEnd) {
    const cur = bfsQueue[bfsStart++];
    const cx = cur % width; const cy = Math.floor(cur / width);
    const tryNeighbor = (idx) => {
      if (exteriorReached[idx]) return;
      const off = idx * channels;
      if (isBackgroundColor(data[off], data[off + 1], data[off + 2])) {
        exteriorReached[idx] = 1;
        bfsQueue[bfsEnd++] = idx;
      }
    };
    if (cx > 0) tryNeighbor(cur - 1);
    if (cx < width - 1) tryNeighbor(cur + 1);
    if (cy > 0) tryNeighbor(cur - width);
    if (cy < height - 1) tryNeighbor(cur + width);
  }

  // Pixels not reached by exterior BFS are the garment — keep them opaque.
  // Apply interior hole-fill: background pixels enclosed by garment (unreachable from border even accounting for garment boundary) should be filled too.
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      ...
      if (exteriorReached[index]) {
        // Background pixel — make transparent.
        outputRaw[off + 3] = 0;
      } else {
        // Garment pixel — keep fully opaque.
        outputRaw[off + 3] = 255;
        if (x < minX) minX = x;
        if (y < minY) minY = y;
        if (x > maxX) maxX = x;
  ...
  const croppedPngBuffer = await sharp(outputRaw, { raw: { width, height, channels } })
  ...
  return {
    ...
    cutout: { width: cropWidth, height: cropHeight, offsetX: minX, offsetY: minY },
}
...
async function createHeuristicGarmentCutout(buffer) {
  ...
  // Skin-tone wearerMask removed — it was incorrectly stripping light-colored sleeves
  // and fabric with warm hues. The flood-fill background removal already handles body pixels.
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      ...
      if (!selectedMask[pixelIndex]) continue;
...
app.post('/analyze-image', upload.any(), async (req, res) => {
  ...
  try {
    ...
    if (req.body.type === 'sizeGuide') {
      ...
      const resolvedGarmentType = resolveRequestedGarmentType(req.body.garmentType, primaryFile.originalname || '');
      console.log('Resolved garment type:', {
        requested: req.body.garmentType || null,
        resolved: resolvedGarmentType,
        fileName: primaryFile.originalname || null,
      });

      const garmentResult = await createGarment3DModel(primaryFile.buffer, resolvedGarmentType, primaryFile.mimetype);
      ...
      console.log('Garment model generation complete:', {
        ...
        stitchPairCount: garmentResult.garmentModel?.stitchPairs ? Math.floor(garmentResult.garmentModel.stitchPairs.length / 2) : 0,
        selfCollisionPairCount: garmentResult.garmentModel?.selfCollisionPairs ? Math.floor(garmentResult.garmentModel.selfCollisionPairs.length / 2) : 0,
        seamDiagnostics: garmentResult.garmentModel?.seamDiagnostics || null,
        framework: garmentResult.garmentModel?.framework || null,
        pipelineVersion: garmentResult.garmentModel?.pipelineVersion || GARMENT_PIPELINE_VERSION,
      });
